  ...
  return (
    <div className="space-y-4">
      <form action={updateAction} className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <label className="text-sm font-medium text-slate-700">Chinese name<input name="nameZh" defaultValue={product.name_zh ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><label className="text-sm font-medium text-slate-700">English name<input name="nameEn" defaultValue={product.name_en ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><label className="text-sm font-medium text-slate-700">Brand<input name="brand" defaultValue={product.brand ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><input type="hidden" name="name" value={product.name} />
        <label className="text-sm font-medium text-slate-700">Category<select name="category" defaultValue={product.category} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3"><option>{product.category}</option><option>飲品</option><option>餅乾</option><option>其他</option></select></label><label className="text-sm font-medium text-slate-700">Flavour<input name="flavour" defaultValue={product.flavour ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><label className="text-sm font-medium text-slate-700">Size<input name="size" defaultValue={product.size ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><label className="text-sm font-medium text-slate-700">Package type<input name="packageType" defaultValue={product.package_type ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label><label className="text-sm font-medium text-slate-700">Barcode<input name="barcode" defaultValue={product.barcode ?? ""} disabled={disabled} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3" /></label>
        <p className="text-sm text-slate-500 sm:col-span-2">Current stock: <strong>{product.current_stock}</strong> (use inventory ledger to change stock)</p>
