    ...
    <div className="min-h-screen px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
      <header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">Good morning, Alice</h1>
          <p className="mt-2 text-sm text-slate-500">Here&apos;s today&apos;s tuck shop overview.</p>
        </div>
        <button className="h-11 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white shadow-lg shadow-blue-700/20 transition hover:bg-blue-800">
          + Record sale
        </button>
      </header>

      <section aria-label="Key metrics" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Today&apos;s revenue" value="RM486.00" detail="↑ 12.4% vs yesterday" tone="blue" />
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
          ...
          <div className="mt-8 flex h-52 items-end gap-2 sm:gap-3" aria-label="Revenue chart preview">
            {[38, 54, 42, 68, 61, 76, 55, 82, 72, 94, 68, 88, 79, 100].map((height, index) => (
              <div key={index} className="flex flex-1 flex-col items-center gap-2">
                <div className="w-full rounded-t-lg bg-blue-100" style={{ height: `${height}%` }}>
                  <div className="h-2/3 w-full rounded-t-lg bg-blue-600" />
                </div>
        ...
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
          ...
          <button className="mt-4 text-sm font-semibold text-amber-900 underline decoration-amber-300 underline-offset-4">
            View inventory
          </button>
        </div>
      </section>
      ...
      <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <button className="self-start text-sm font-semibold text-blue-700 hover:text-blue-800 sm:self-auto">View report →</button>
        </div>
              ...
              {products.map((product) => (
                <tr key={product.name}>
                  <td className="py-4 font-medium text-slate-800">
                    <span className={`mr-3 inline-block size-2 rounded-full ${product.colour}`} />
                    {product.name}
                  </td>
                  <td className="py-4 text-slate-500">{product.sold}</td>
                  <td className="py-4 text-right font-semibold text-slate-800">{product.revenue}</td>
                </tr>
              ))}
            </tbody>
          </table>
