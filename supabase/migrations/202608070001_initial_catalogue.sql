-- Initial test catalogue for the first connected environment.
-- Keep this idempotent so a partially seeded project can be repaired safely.
...
select sample.product_code,
  ...
  sample.current_stock,
  sample.minimum_stock
from (values
  ...
  ('P010', 'Sandwich', 'Food', 2.20::numeric, 3.50::numeric, 10, 5)
) as sample(product_code, name, category, cost_price, selling_price, current_stock, minimum_stock)
where not exists (
  select 1
  from public.products product_row
  where lower(product_row.product_code) = lower(sample.product_code)
);
